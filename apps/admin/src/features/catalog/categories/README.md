...
| 11  | `sticky` table header scrolled away with the page.                                    | `Table`'s `overflow-x-auto` wrapper is also a _vertical_ scroll container (CSS coerces `overflow-y` to `auto`), so `sticky top-0` pinned to that non-scrolling box. `Table scrollX={false}` → `overflow-x-clip` (clips corners, not a scroll container); the tree tables use it since they're `table-fixed`. |
