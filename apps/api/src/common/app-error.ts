...
export interface AppErrorFieldIssue {
  field: string;
  rule: string;
  /** i18n key, not English (plan/CODING-RULES.md §L1). */
  message: string;
}

export interface AppErrorOptions {
  /** Developer-facing context. Never shown to end users (plan/CODING-RULES.md §F2). */
  detail?: string;
 ...
 * global filter turns it into the RFC-9457 envelope (plan/08 §4). Anything else
