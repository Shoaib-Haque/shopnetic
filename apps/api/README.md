...
| GET            | `/identity/v1/audit-events`                                               | **staff Bearer** + `auditlog:read` — newest-first, cursor-paginated (`?cursor=&limit=`)                                                                                                                                                                                                                                                                                       |
...
| POST           | `/identity/v1/staff/auth/change-password`                                 | **staff Bearer** (no permission needed — self only) — `{ currentPassword, newPassword }` → `204`, revokes every session                                                                                                                                                                                                                                                       |
| POST           | `/identity/v1/staff/auth/forgot-password`                                 | `{ email }` → always `202` (enumeration-safe); emails a reset link when the account exists                                                                                                                                                                                                                                                                                    |
| POST           | `/identity/v1/staff/auth/reset-password`                                  | `{ token, newPassword }` → `204`, revokes every session; `400`/`410` on an invalid/expired token                                                                                                                                                                                                                                                                              |
...
| POST           | `/identity/v1/staff/:accountId/activate`                                  | **staff Bearer** + `staff:manage` — `locked`/`disabled` → `active`; `422` if already active                                                                                                                                                                                                                                                                                   |
...
| \*             | `/admin/v1/categories` (+ `…/:id`, `…/:id/move`)                          | **staff Bearer** + `category:manage` — category tree CRUD (ltree `path`, reparent, soft-delete). `GET` supports `?status=&q=&cursor=&limit=` — omit `limit` for the whole matching set (the active tree's own load), pass it to cursor-paginate (the flat Archived/All views); `q` searches server-side, same token/OR/score matcher the admin client used to run client-side |
...
`AUTH_STAFF_REFRESH_TTL_HOURS`, `VERIFICATION_TTL_HOURS`,
